...
class Experiment:
    ...
    def train_and_eval(self):
        ...
        # train_data_idxs = torch.tensor(np.array(train_data_idxs))
        ...
        for it in bar:
            ...
            for j in range(0, len(train_data_idxs), self.batch_size):
                ...
                e1_idx = data_batch[:, 0]
                r_idx = data_batch[:, 1]
                e2_idx = torch.cat([data_batch[:, 2:3], negsamples], dim=-1) #第一列全为正确的
            ...
            with torch.no_grad():
                if not it % self.valid_steps:
                    ...
                    print(
                        '\nValid Best it:%d  Hit@10:%f  Hit@3:%f  Hit@1:%f  MRR:%f' %
                        (max_it, hit10, hit3, hit1, mrr))
                    thit10, thit3, thit1, tmrr = self.evaluate(model, d.test_data)
                    print(
                        'Test Best it:%d  Hit@10:%f  Hit@3:%f  Hit@1:%f  MRR:%f\n' %
                        (max_it, thit10, thit3, thit1, tmrr))
                    if mrr > max_mrr:
                        max_mrr = mrr
                        max_it = it
                        bad_cnt = 0
                        ...
                        if bad_cnt == self.early_stop:
                            break
                    bar.set_description('Best MRR:%.4f@%d,cur:%.4f,loss:%.3f' %
